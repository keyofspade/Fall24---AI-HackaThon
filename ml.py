...
load_dotenv('/flask-app/language.env')
...
# Merge `X_train` with `recommendations_df` to create training data 
training_data = X_train.merge(recommendations_df, on='category', how='left')

# Creating function to create and train RF model on previously made training data
def train_rf_classifier(training_data, target_column):
    if target_column not in training_data.columns:     # Check if the target column exists
    ...
    # Applying predictions made from the model into y_pred object
...
# Train models for Action and Recommendation, storing predictions using previously made function
...
# Creating a new df to hold predictions
training_prediction = training_data[['entity', 'category', 'Action', 'Recommendation']].copy()

# Adding action and recommendation predictions to the new DataFrame
training_prediction['predicted_action'] = action_predictions['predicted_action'].values
training_prediction['predicted_recommendation'] = action_predictions['predicted_recommendation'].values
...
blob_client_csv = blob_service_client.get_blob_client(container="ingest-dashboard", blob=blob_name_csv) 
